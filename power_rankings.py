    ...
    for player in current_strength_per_player:
        ...
        with open('temp/{}_stats.txt'.format(player)) as player_history_file:
            for game in player_history_file:
                ...
                [replay, num_players, score, variant, time, players, other_games_same_seed, num_suits, max_score] = game
                
                fraction_max_score = int(score) / int(max_score)
                players = players.split(', ')
                if all([player in current_strength_per_player for player in players]):
                    game = { 'fraction_max_score': fraction_max_score, 'variant': variant, 'players': players }
                    games[replay] = game
                    num_games += 1
...
for iteration in range(100):
    sum_performances_per_player = dict([(player, 0) for player in current_strength_per_player])
    for replay in games:
        game = games[replay]

        sum_strengths = sum([current_strength_per_player[player] for player in game['players']])
        for alice in game['players']:
            sum_strengths_excluding_alice = sum_strengths - current_strength_per_player[alice]
            avg_strength_excluding_alice = sum_strengths_excluding_alice / (len(game['players']) - 1)

            alice_performance = game['fraction_max_score'] - avg_strength_excluding_alice

            sum_performances_per_player[alice] += alice_performance

    next_strength_per_player = {}
    for player in sum_performances_per_player:
        next_strength_per_player[player] = sum_performances_per_player[player] / num_games_per_player[player]

    # # Normalize next strengths to be between 0 and 1
    # min_next_strength = min(next_strength_per_player.values())
    # max_next_strength = max(next_strength_per_player.values())
    # next_strength_interval = max_next_strength - min_next_strength

    # for player in next_strength_per_player:
    #     next_strength_per_player[player] -= min_next_strength
    #     next_strength_per_player[player] /= next_strength_interval
    #     assert next_strength_per_player[player] >= 0 and next_strength_per_player[player] <= 1

    current_strength_per_player = next_strength_per_player

    with open('output/iteration_{}.tsv'.format(iteration), 'w') as output:
        for [player, current_strength] in sorted(current_strength_per_player.items(), key = lambda item: item[1], reverse = True):
